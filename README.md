...
- 🖼️ **Inline images (Sixel)** — programs that emit Sixel (`img2sixel`, `chafa -f sixel`, matplotlib's sixel backend, `lsix`) render a **real bitmap right in the grid**, GPU-textured, at native size, scrolling with your scrollback. Image previews and plots without leaving the terminal.
